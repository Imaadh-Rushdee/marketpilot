...
function ContentEditor({content,userId,updateContent,regenerateContent,deleteContent,notify,onGraphics,loading}:{content:ContentItem;userId:string;updateContent:(id:string,p:Partial<ContentItem>)=>void;regenerateContent:(c:ContentItem)=>void;deleteContent:(id:string)=>void;notify:(message:string)=>void;onGraphics:()=>void;loading:boolean}) {
  ...
  return <section className="card editor">
    ...
    {content.status!=="Posted"&&<fieldset className="field platform-field"><legend>Publish to</legend><div className="chips">{PLATFORMS.map(p=><button type="button" key={p} aria-pressed={destinations(content).includes(p)} className={destinations(content).includes(p)?"chip selected":"chip"} onClick={()=>{const selected=destinations(content);const next=selected.includes(p)?selected.filter(x=>x!==p):[...selected,p];if(!next.length){notify("Select at least one destination.");return;}updateContent(content.id,{platforms:next,platform:next[0],status:"Draft"});}}>{p}</button>)}</div></fieldset>}
