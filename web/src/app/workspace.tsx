        ...
        {view === "studio" && (
          campaigns.length === 0 ? <EmptyAction text="Create a campaign first to open the Content Studio." action={() => setView("create")}/> : (
            <section className="studio-layout">
              ...
              {activeContent && <ContentEditor content={activeContent} updateContent={updateContent} regenerateContent={regenerateContent} deleteContent={deleteContent} notify={setMessage} onGraphics={()=>setView("graphics")} loading={loading}/>} 
            </section>
          )
        )}
...
function ContentEditor({content,updateContent,regenerateContent,deleteContent,notify,onGraphics,loading}:{content:ContentItem;updateContent:(id:string,p:Partial<ContentItem>)=>void;regenerateContent:(c:ContentItem)=>void;deleteContent:(id:string)=>void;notify:(message:string)=>void;onGraphics:()=>void;loading:boolean}) {
  return <section className="card editor">
    ...
    <div className="two-col"><Field label="Publish date"><input type="date" value={content.date} onChange={e=>{if(e.target.value) updateContent(content.id,{date:e.target.value});}}/></Field><div className="approval-actions"><span className={`status ${content.status.toLowerCase()}`}>{content.status}</span><button className="primary" disabled={content.status !== "Draft"} onClick={()=>updateContent(content.id,{status:"Approved"})}><Check size={17}/>Approve post</button>{content.status === "Approved" && <button className="chip" onClick={()=>{if(confirm("Have you published this post to every selected platform?"))updateContent(content.id,{status:"Posted"});}}>Mark posted on all platforms</button>}{content.status !== "Draft" && <button className="text-btn" onClick={()=>updateContent(content.id,{status:"Draft"})}>Return to draft</button>}</div></div>
    <p className="muted">Use this post on every platform shown above. Publish manually to each account, then mark it posted. Editing the copy returns it to draft for review.</p><div className="editor-save"><span><Pencil size={15}/> Changes sync automatically</span></div>
  </section>;
}
