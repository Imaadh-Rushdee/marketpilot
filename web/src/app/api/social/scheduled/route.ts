...
export async function POST(request:Request){
 ...
 for(const row of rows||[]){if(!validCampaigns(row.campaigns))continue;let changed=false;const campaigns=structuredClone(row.campaigns) as Campaign[];
  for(const campaign of campaigns)for(const content of campaign.contents){if(published>=1||!due(content,now.date,now.time))continue;const platforms=destinations(content);if(platforms.some(p=>p!=="Facebook"&&p!=="Instagram")){console.warn("Scheduled post has unsupported destinations",{userId:row.user_id,contentId:content.id,platforms});failed++;continue;}let temporaryPath="";
 ...
 return NextResponse.json({ok:failed===0,published,failed,checkedAt:now,timeZone:zone},{status:failed&&published===0?502:200});
}
